...
# 路径集合#
path_dic = {'foreign_data':r"C:\Users\Mocilly\Desktop\研创平台课题项目\数据\factset\data\时间趋势分析数据",
...
files[1]
df_cop = pd.read_stata(files[1]) #公司df 
df_sc = pd.read_stata(files[2]) #供应链df
...
df_sc_copy = pd.read_csv(r'.\调用文件\统计在华外资企业的可视化表\8.新算法_添加所属国家后的供应链关系表.csv')
...
copy_index_list = []
new_index_list = []
# 判断表格中新增的行的索引（利用时间判别：如果start_和end_都在2016年之前，则将其计入new_index_list，
# 否则将df_sc_copy的索引与df_sc的索引组成列表添加到copy_index_list中）
length_sc = len(df_sc.index)
copy_start_index = 0
for i in df_sc.index:
    start_time = pd.to_datetime(df_sc.loc[i, 'start_'], errors='coerce')
    end_time = pd.to_datetime(df_sc.loc[i, 'end_'], errors='coerce')
    
    if start_time.year < 2016 and end_time.year < 2016:
        new_index_list.append(i)
    else:
        copy_index_list.append([i,copy_start_index])
        copy_start_index += 1
    print(f"已解决{i+1}/{length_sc}")
...
c = 0
length_copy = len(copy_index_list)
for copy_index in copy_index_list:
    sc_index = copy_index[0]
    sc_copy_index = copy_index[1]
    df_sc.loc[sc_index,'source_company_belong'] = df_sc_copy.loc[sc_copy_index,'source_company_belong']
    df_sc.loc[sc_index,'target_company_belong'] = df_sc_copy.loc[sc_copy_index,'target_company_belong']
    print(f"已解决{c+1}/{length_copy}")
    c += 1
...
df_sc_copy.columns
df_cop.columns
length_new_list = len(new_index_list)
ccc = 0
for index in new_index_list:

    source_cop_name = df_sc.loc[index,'source_company_id']
    condition_cop = (df_cop['id'] == source_cop_name)
    ...
    if len(column_set) == 1:
        ...
        if len(column_set_1) == 1 : #如果home_region不为空，则取出home_region
            ...
            temp_str = '|'.join([country for country in list(column_set_1) if country.strip() != ""])
            sc_status = f'home_region:{temp_str}&'
        if len(column_set_2) == 1: #如果country不为空，则取出country
            ...
            temp_str = '|'.join([country for country in list(column_set_2) if country.strip() != ""])
            sc_status += f'country:{temp_str}'
    else:
        sc_status = 'Nation_Not_Found'

    df_sc.loc[index,'source_company_belong'] = sc_status
    print(f"已解决{ccc+1}/{length_new_list}")
    ccc += 1
...
for index in new_index_list:
    ...
    condition_cop = (df_cop['id'] == target_cop_name)
